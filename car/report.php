    ...
    <div class="page-heading">
        <div class="page-heading-copy">
            ...
            <div>
                ...
                <h1 class="h3 mb-1"><?= $is_filtered ? bn_month_label($selected_month, $bn_months) . ' এর রিপোর্ট' : 'সার্বিক রিপোর্ট' ?></h1>
                <p class="text-muted mb-0 small">
                    <?= $is_filtered ? 'এই মাসে' : 'সর্বমোট' ?> <?= $total_records ?> টি রেকর্ড
                    <?php if (!$is_filtered): ?> &middot; আজকের নতুন এন্ট্রি: <?= count($today_records) ?> টি<?php endif; ?>
                </p>
            </div>
        </div>
    ...
    <div class="panel mb-4">
        ...
        <form method="GET" class="month-filter-body">
            ...
            <div class="month-filter-field">
                ...
                <input type="month" id="monthInput" name="month" value="<?= htmlspecialchars($selected_month) ?>" max="<?= date('Y-m') ?>">
            </div>
            <button type="submit" class="btn btn-primary">
                <i class="bi bi-search"></i> দেখুন
            </button>
            <?php if ($is_filtered): ?>
                <a href="?<?= http_build_query(array_diff_key($_GET, ['month' => ''])) ?>" class="btn btn-outline-secondary">
            ...
            <div class="month-quick-links">
                <?php foreach (array_slice($available_months, 0, 6) as $ym): ?>
                    <a href="?month=<?= urlencode($ym) ?>" class="month-chip <?= $ym === $selected_month ? 'active' : '' ?>">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-6">
            <div class="progress-card">
                ...
                <span class="progress-note"><?= $collection_rate ?>% আদায় হয়েছে &middot; বাকি ৳ <?= number_format($total_due, 0) ?></span>
            </div>
        </div>
    </div>
            ...
            <table class="table table-hover align-middle mb-0" id="reportTable">
                ...
                <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="cust-cell">
                                <span class="cust-avatar"><?= htmlspecialchars(mb_substr($row['customer_name'] ?? '?', 0, 1)) ?></span>
                        ...
                        <td class="text-end text-success fw-semibold">৳ <?= number_format($row['paid_amount'] ?? 0, 2) ?></td>
                        <td class="text-end text-danger fw-semibold">৳ <?= number_format($row['due_amount'] ?? 0, 2) ?></td>
                        <td>
                            <span class="badge-status badge-<?= ($row['status'] ?? '') == 'completed' ? 'success' : 'warning' ?>">
...
.report-page .stat-body{display:flex;flex-direction:column;}
.report-page .stat-label{font-size:12.5px;color:var(--rp-muted);font-weight:600;}
.report-page .stat-value{font-size:20px;font-weight:700;color:#1e293b;}
...
.report-page .progress-bar{border-radius:6px;}
...
@media print {
    ...
    .report-page .progress-card,
    .report-page .month-filter-body,
    .report-page .search-wrap { display: none !important; }
    .report-page .panel { border: 1px solid #ccc; box-shadow:none; }
    .report-page table thead{background:#000 !important;-webkit-print-color-adjust:exact;print-color-adjust:exact;}
}
</style>