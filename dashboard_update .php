...
$stmt = $pdo->query("
    ...
    FROM rents
    WHERE MONTH(rent_date) = MONTH(CURDATE())
    AND YEAR(rent_date) = YEAR(CURDATE())
    AND TRIM(payment_status) = 'paid'
");


$totalRent = (float)($stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0);

$stmt = $pdo->prepare("
    SELECT COALESCE(SUM(rent_amount), 0) AS total
    FROM rents
    WHERE DATE(rent_date) = ?
    AND TRIM(payment_status) = 'paid'
");
$stmt->execute([$today]);
$todayRent = (float)($stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0);

$stmt = $pdo->query("
    SELECT COALESCE(SUM(rent_amount), 0) AS total
    FROM rents
    WHERE rent_date >= DATE_FORMAT(CURDATE(), '%Y-%m-01')
      AND rent_date < DATE_ADD(
          DATE_FORMAT(CURDATE(), '%Y-%m-01'),
          INTERVAL 1 MONTH
      )
      AND TRIM(payment_status) = 'due'
");

$dueRent = (float)($stmt->fetchColumn() ?? 0);
 // =====================================================
// totalIncomevv Bile
// =====================================================
...
$stmt = $pdo->query("
    ...
    WHERE MONTH(transaction_date) = MONTH(CURDATE())
    AND YEAR(transaction_date) = YEAR(CURDATE())
     AND TRIM(type) = 'expense'
   
");
$totalExpense = (float)($stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0);
...
$stmt = $pdo->prepare(" SELECT COUNT(*) AS total FROM metro_cars");
$stmt->execute();
$totalMetroCars = (int)($stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0);
...
$stmt = $pdo->query("
    ...
    WHERE MONTH(payment_date) = MONTH(CURDATE())
    AND YEAR(payment_date) = YEAR(CURDATE())
    AND TRIM(payment_type) = 'monthly'
     
   
");
$metroCollection = (float)($stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0);
$metroDue         = $metroDue ?? 0;
...
$stmt->execute();
$totalEmployees = (int)($stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0);


$stmt = $pdo->prepare("  SELECT COUNT(*) AS total FROM employee_attendance WHERE DATE(created_at) = ? AND status = 'present' ");
$stmt->execute([$today]);
$presentEmployees = (int)($stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0);
$stmt = $pdo->prepare("  SELECT COUNT(*) AS total FROM employee_attendance WHERE DATE(created_at) = ? AND status = 'absent' ");
$stmt->execute([$today]);
$absentEmployees = (int)($stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0);
...
");
$salaryPaid = (float)($stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0);
